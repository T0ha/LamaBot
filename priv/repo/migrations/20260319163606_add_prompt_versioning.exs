defmodule Bodhi.Repo.Migrations.AddPromptVersioning do
  use Ecto.Migration

  def up do
    execute(versioning_function_sql())

    alter table(:prompts) do
      add :sys_period, :tstzrange,
        null: false,
        default: fragment("tstzrange(now(), null)")
    ...
    create table(:prompts_history, primary_key: false) do
    ...
    FOR EACH ROW EXECUTE PROCEDURE versioning(
      'sys_period', 'prompts_history', true, true,
      false, false, true, 'version'
    );
    """)
    ...
    UPDATE prompts
    SET version = 1,
        sys_period = tstzrange(inserted_at::timestamptz, null)
    WHERE version = 1;
    """)
  end

  def down do
    execute("DROP TRIGGER IF EXISTS prompts_versioning_trigger ON prompts;")

    drop_if_exists table(:prompts_history)
    ...
    execute("DROP FUNCTION IF EXISTS versioning();")
  end

  defp versioning_function_sql do
    """
    -- version 1.2.1
